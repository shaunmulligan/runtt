...
fn repo_root() -> &'static Path {
    // CARGO_MANIFEST_DIR is crates/mcu-runtime.
...
fn firmware_default_version() -> String {
    let kconfig = read("firmware/balena-mcu/zephyr/Kconfig");
    let mut lines = kconfig.lines();
    while let Some(l) = lines.next() {
        if l.trim() == "config BALENA_MCU_CONTRACT_VERSION" {
    ...
    panic!("no default found for BALENA_MCU_CONTRACT_VERSION");
}

/// The major the runtime is willing to talk to.
fn runtime_major() -> u32 {
    let src = read("crates/mcu-runtime/src/flash.rs");
    src.lines()
        .find_map(|l| {
            let l = l.trim();
...
fn the_document_and_the_firmware_agree() {
    assert_eq!(
        ...
        "docs/WIRE_CONTRACT.md and BALENA_MCU_CONTRACT_VERSION disagree. \
...
fn the_describe_group_is_the_per_user_base() {
    ...
    assert!(
        ...
        "BALENA_MCU_SMP_GROUP_ID should default to 64 (MGMT_GROUP_ID_PERUSER)"
    );
    assert_eq!(smp_client::describe::GROUP_PERUSER, 64);
}

#[test]
fn the_documented_interface_strings_are_the_ones_we_match_on() {
    let doc = read("docs/WIRE_CONTRACT.md");
    for s in [transport::usb::IFACE_MGMT, transport::usb::IFACE_LOG] {
        ...
        let rules = read("udev/90-balena-mcu.rules");
        assert!(rules.contains(s), "udev rules should match on {s}");
        // As should the hardware overlay that produces them.
        let overlay = read("firmware/balena-mcu/snippets/balena-mcu/boards/rpi_pico.overlay");
        assert!(
            overlay.contains(s),
