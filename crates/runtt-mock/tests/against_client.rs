...
//!
//! This is the load-bearing test of phase 0. The framing in `runtt-mock` and the
...
fn echo_round_trips_through_our_own_framing() {
    ...
    let got = c.echo("runtt").expect("echo should succeed");
    assert_eq!(got, "runtt", "the device must echo the payload verbatim");
}
...
fn progress_is_reported_monotonically_to_completion() {
    ...
    impl runtt_smp::Progress for Rec {
