...
const RUNTIME: &str = env!("CARGO_BIN_EXE_runtt");
...
const SIGNED: &[u8] = include_bytes!("../../runtt-smp/tests/fixtures/app.signed.bin");
const SIGNED_OTHER: &[u8] = include_bytes!("../../runtt-smp/tests/fixtures/other.signed.bin");
...
const SIGNED_SMALL: &[u8] = include_bytes!("../../runtt-smp/tests/fixtures/small.signed.bin");
...
fn spec_json(target: &str) -> String {
  ...
  "annotations": {{ "dev.runtt.target": "{target}" }}
}}"#
    )
}
...
fn a_digest_mismatch_fails_the_container() {
    ...
    let text = rig.wait_for(&log, "runtt:", Duration::from_secs(60));
    assert!(
...
fn a_missing_target_annotation_is_refused_at_create() {
    ...
    // A spec with no dev.runtt.target at all.
    std::fs::write(
        bundle.join("config.json"),
        r#"{"ociVersion":"1.2.0",
...
fn force_reflash_overrides_the_skip() {
    ...
    std::fs::write(
        ...
        spec_json_with(
            ...
            r#","dev.runtt.skip-if-same-hash":"false""#,
        ),
    )
    .unwrap();
...
fn spec_json_with(target: &str, extra_annotations: &str) -> String {
  ...
  "root": {{ "path": "rootfs", "readonly": true }},
  "annotations": {{ "dev.runtt.target": "{target}"{extra_annotations} }}
}}"#
    )
}
...
fn firmware_without_describe_still_deploys_but_warns() {
    let _seq = serial();
    // What a customer gets if they build without the runtt module, or with
